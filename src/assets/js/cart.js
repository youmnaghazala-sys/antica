...
class Cart extends BasePage {
  ...
  updateCartPageInfo(cartData) {
    ...
    app.toggleElementClassIf(
      ...
      "active",
      "hidden",
      () => cartData?.gift?.enabled
    );
    // Use toggleAttribute to handle the `physical-products` attribute
    app.sallaGifting?.toggleAttribute(
      "physical-products",
      cartData?.gift?.type === "physical"
    );
    app.sallaGifting?.toggleAttribute(
      "digital-products",
      cartData?.gift?.type === "digital"
    );
    ...
    if (app.orderOptionsTotal)
      app.orderOptionsTotal.innerHTML = salla.money(cartData.options_total);
    ...
    app
      ...
      .toggleElementClassIf(
        ...
        "has_shipping",
        "hidden",
        () => !!cartData.real_shipping_cost
      )
      .toggleElementClassIf(
        app.freeShipping,
        "has_free",
        "hidden",
        () => !!cartData.free_shipping_bar
      );

    app.totalDiscount.querySelector("b").innerHTML =
      "- " + salla.money(cartData.total_discount);
    app.shippingCost.querySelector("b").innerHTML = salla.money(
      cartData.real_shipping_cost
    );
    ...
    app.freeShippingBar.children[0].style.width =
      cartData.free_shipping_bar.percent + "%";
  }
  ...
  updateItemInfo(item) {
    ...
    let item_total =
      item.detailed_offers?.length > 0 ? item.total_special_price : item.total;
    let total = salla.money(item_total);
    if (total !== totalElement.innerHTML) {
      totalElement.innerHTML = total;
      app.anime(totalElement, { scale: [0.88, 1] });
    }

    app
      .toggleElementClassIf(
        [offerElement, oldOffers],
        "offer-applied",
        "hidden",
        () => hasSpecialPrice && !newOffersActive
      )
      .toggleElementClassIf(
        [regularPriceElement, offerIconElement],
        "offer-applied",
        "hidden",
        () => hasSpecialPrice
      )
    ...
    app.onClick(app.couponBtn, (event) => {
      ...
      if (!hasCoupon && !app.couponCodeInput.value.length) {
        this.showCouponError(
          "* " + salla.lang.get("pages.checkout.enter_coupon")
        );
        return;
      }
      btn
        ...
        .then(() =>
          hasCoupon
            ? salla.cart.deleteCoupon()
            : salla.cart.addCoupon(app.couponCodeInput.value)
        )
        .then((res) => this.toggleCoupon(res, !hasCoupon))
        .catch((err) =>
          this.showCouponError(err.response?.data?.error.message, !hasCoupon)
        )
        .finally(() => btn.stop());
    });
  }
  ...
  toggleCoupon(_res, applied) {
    ...
    app
      .toggleElementClassIf(
        app.couponBtn,
        ["btn--danger", "has-coupon"],
        ["btn-default", "has-not-coupon"],
        () => applied
      )
      .toggleElementClassIf(
        app.couponBtn,
  ...
  showCouponError(message, isApplying = true) {
    app.couponError.innerText =
      message || salla.lang.get("pages.checkout.error_occurred");
